...
<?php
    ...
    if( isset($_POST['delete_order'])){
        ...
        if($verify_delete->rowcount() > 0){
            ...
            $delete_message = $conn->prepare("DELETE FROM `orders` WHERE order_id = ?");
            $delete_message->execute([$order_id]);
...
<head>
    ...
    <title>green coffe admin panel - confirm order page</title>
</head>
<body>
...
<div class="title2">
    <a href="dashboard.php">dashboard</a><span> / confirm orders</span>
</div>

    <section class ="order-container">
        <h1 class="heading">total confirm orders</h1>
        <div class="box-container">
            <?php
            $select_orders = $conn->prepare("SELECT * FROM `orders` where status = 'complete' ");
