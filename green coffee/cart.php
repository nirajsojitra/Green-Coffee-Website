<?php
    ...
    if (isset($_POST['delete_item'])) {
        ...
        $varify_delete_items = $conn->prepare("SELECT * FROM `cart` WHERE cart_id = ?");
        $varify_delete_items->execute([$cart_id]);

        if ($varify_delete_items->rowCount()>0) {
            $delete_wishlist_id = $conn->prepare("DELETE FROM `cart` WHERE cart_id = ?");
            $delete_wishlist_id->execute([$cart_id]);
        ...
        if (isset($_POST['empty_cart'])) {
            ...
            if ($varify_empty_item->rowCount() > 0) {
                $delete_empty_id = $conn->prepare("DELETE FROM `cart` WHERE user_id = ?");
                $delete_empty_id->execute([$user_id]);
     ...
     <div class="banner">
        ...
        <section class="products">
            ...
            <div class="cart-total">
                <p>total amount payble : <span>₹ <?= $grand_total; ?>/-</span></p>
                <div class="button">
